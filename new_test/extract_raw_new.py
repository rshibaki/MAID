...
input_path = f"new_test/outputs/raw_data/{args.model}_{args.prompt}_outputs.jsonl"
output_path = f"new_test/outputs/extract_data/{args.model}_{args.prompt}_extracted.csv"
...
def extract_answer(text, prompt):
    ...
    """
    if prompt in ("control", "cot", "mukankei"):
        # 改行を統一
        ...
        answer_match = re.search(r"Answer:\s*(Option lion|Option tiger|Option 1|Option 2|Option ♢|Option ☆)?", text, re.IGNORECASE)
        reason_match = re.search(r"Reason:\s*(.*)", text, re.IGNORECASE)
        answer_pre = answer_match.group(1).strip() if answer_match and answer_match.group(1) else ""
        answer = mapping.get(answer_pre.lower(), "")
        reason = reason_match.group(1).strip() if reason_match else ""
...
with open(input_path, "r", encoding="utf-8") as infile, \
    ...
    for line in infile:
        ...
        answer_choice, reason = extract_answer(output_text, args.prompt)
...
# # 空白回答だけを抽出して保存
# blank_output_path = "new_test/llama3_blank_answers.jsonl"
...
#                 "answer_meaning": row["answer_meaning"],
#                 "reason": row["reason"]
#             }
